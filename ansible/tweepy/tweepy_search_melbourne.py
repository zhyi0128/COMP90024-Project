...
import new_extract_suburb_scenario
...
SCENARIO_SUBURBS_DATABASE_NAME = 'scenario_suburb'
...
def create_couchdb():
    ...
    try:
        scenario_suburbs_database = server.create(
            SCENARIO_SUBURBS_DATABASE_NAME)
    except:
        scenario_suburbs_database = server[SCENARIO_SUBURBS_DATABASE_NAME]

    return original_database, scenario_database, scenario_suburbs_database
...
if __name__ == '__main__':
    greater_citys_id = tweepy_tokens.greater_citys_id['melbourne']
    tweepy_api = tweepy_tokens(greater_citys_id=greater_citys_id)

    original_database, scenario_database, scenario_suburbs_database = create_couchdb()

    # set date
    start_time = datetime.date.today() - datetime.timedelta(days=7)
    end_time = datetime.date.today()

    # melbourne latitude longitude
    ...
    while True:
        try:
            ...
            scenario_dic = scenario_cities.execute_melbourne(
                original_dic['_id'], original_dic['full_text'])
            
            scenario_suburbs_dic = new_extract_suburb_scenario.execute(
                original_dic)

            if scenario_dic != {}:
                scenario_database.save(scenario_dic)

            if scenario_suburbs_dic != {}:
                scenario_suburbs_database.save(scenario_suburbs_dic)
